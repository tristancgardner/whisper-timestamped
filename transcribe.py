...
os.environ["TF_CPP_MIN_LOG_LEVEL"] = (
    "1"  # Remove warning "This TensorFlow binary is optimized with oneAPI Deep Neural Network Library (oneDNN)..."
)
...
##! add this back on intel processor on PC
...
def transcribe_timestamped(
    ...
    ), f"word_alignement_most_top_layers must be a strictly positive number"
    ...
    alignment_heads = (
        get_alignment_heads(model) if word_alignement_most_top_layers is None else None
    )
    ...
    if vad:
        ...
        audio, convert_timestamps = remove_non_speech(
            audio, method=vad, plot=plot_word_alignment
        )

    global num_alignment_for_plot
    num_alignment_for_plot = 0

    ## THIS IS THE FIRST INSERTION TO TRANSCRIBE
    if naive_approach:
        (transcription, words) = _transcribe_timestamped_naive(
            model,
            audio,
    ...
    ensure_increasing_positions(
        words, min_duration=min_word_duration if trust_whisper_timestamps else 0
    )
    ...
    for word in words:
        ...
        assert idx_segment < len(
            whisper_segments
        ), f"Fatal error: Got unexpected segment index {idx_segment} >= {len(whisper_segments)}"
...
def _transcribe_timestamped_efficient(
    ...
    **whisper_options,
    ...
    whisper_options["verbose"] = (
        None if whisper_options["verbose"] is True else whisper_options["verbose"]
    ...
    last_chunk_token = (
        None  # last token of the current chunk, that may be needed for corner cases
    )
    last_token_fallback = (
        None  # last token to use as a fallback if the model gets stuck
    )
    has_started = False  # whether we have started decoding
    mfcc = None  # MFCC features for the current 30 sec chunk
    new_mfcc = None  #
    num_inference_steps = (
        0  # number of inference steps performed so far (for debugging only)
    )
    ...
    def reset(add_segment, keep_last_token=True):
        ...
        if add_segment:
            ...
            if debug:
                logger.debug(
                    f"Reset last segment: {tokenizer.decode_with_timestamps(segment_tokens[-1])}"
                )
            segment_tokens[-1] = []
            segment_attweights = [[] for w in segment_attweights]
    ...
    def must_flush_segment(curr_tokens):
        ...
        if not is_sot(curr_tokens):
            ...
            if (
                not must_flush and WHIPSER_GE_20230306
    ...
    def align_last_segment(curr_tokens=None):
        ...
        if debug and trust_whisper_timestamps:
            logger.debug(
                f"Add segment {len(timestamped_word_segments)+1} at step {num_inference_steps}:\n\t{tokenizer.decode_with_timestamps(segment_tokens[-1])}"
            )
        ...
        if unfinished_decoding:
            logger.debug(
                f"WARNING: decoding hit the max limit for segment {segment_tokens[-1]} (It usually happens when the language model gets stuck)"
            )
            ...
            last_is_not_timestamp
        ):  # <eot> was emitted early, without a timestamp before
            ...
            if not trust_whisper_timestamps:
                ...
                if added:
                    if len(consecutive) > 1:
                        ...
                        if debug:
                            logger.debug(
                                f"Add segment {len(timestamped_word_segments)}:\n\t{tokenizer.decode_with_timestamps(segment)}"
                            )
    ...
    def hook_attention_weights(layer, ins, outs, index):
        ...
        assert (
            isinstance(outs, tuple) and len(outs) == 2
        ), "whisper seems to be outdated, please update it (pip install --upgrade --no-deps --force-reinstall git+https://github.com/openai/whisper.git)"
        if not has_started:
            return
        w = outs[-1]
        ...
        if is_sot(curr_tokens):
            ...
            if language is None:
                if len(curr_tokens) > 1:
                    ...
                    if (
                        verbose
                        and not whisper_options["verbose"]
                        and len(curr_tokens) > 1
                    ):
    ...
    def hook_output_logits(layer, ins, outs):
        ...
        if language is None and sot_index is not None and model.is_multilingual:
            ...
            language_probs = dict(
                zip(whisper.tokenizer.LANGUAGES, language_probs.tolist())
            )
    ...
    for i, (segment, timestamped_words, token, avglogprob, logprobs) in enumerate(
        ...
        if timestamped_tokens != whisper_tokens:
            if len(timestamped_tokens) == len(whisper_tokens) + 1:
                ...
                ), f"Fatal Error: Got empty segment {i} with non-zero duration"
                ...
                segment["tokens"] = (
                    token if WHIPSER_GE_20230306 else timestamped_tokens
                )  # tokens include special timestamp tokens since 20230306
            ...
            for timestamped_word in timestamped_words:
                ...
                if include_punctuation_in_confidence:
                    ...
                    while (
                        len(tokens) > 1
                        and len(tokens[-1])
                        and tokens[-1][-1] in _punctuation
                ...
                timestamped_word["confidence"] = round_confidence(
                    word_logprobs.mean().exp().item() if len(word_logprobs) else 0.0
                )
...
def _transcribe_timestamped_naive(
    ...
    **whisper_options,
    ...
    language = whisper_options["language"]
    refine_whisper_precision_sec = (
        refine_whisper_precision_nframes * AUDIO_TIME_PER_TOKEN
    )
    ...
    try:
        transcription = model.transcribe(audio, callback=callback, **whisper_options)

        # print("\n\nTranscription complete...re-aligning word-level timestamps\n")
    ...
    try:
        ...
        previous_end = 0
        whisper_segments = transcription["segments"]
        segments_length = len(whisper_segments)
        ...
        for i_segment, segment in enumerate(whisper_segments):
            ...
            if trust_whisper_timestamps:
                ...
                if start >= audio_duration - min_word_duration or (
                    previous_end >= start_margin_min
                    and previous_end <= start_margin_max
                ):
                ...
                if i_segment < len(whisper_segments) - 1:
                    ...
                    end_margin_max2 = (
                        whisper_segments[i_segment + 1]["start"]
                        + refine_whisper_precision_sec
                        - min_word_duration
                    )
                ...
                if new_tokens[0] < tokenizer.timestamp_begin:
                    relative_start = segment["start"] - (
                        seek * HOP_LENGTH / SAMPLE_RATE
                    )
                    start_token = (
                        round(relative_start * SAMPLE_RATE / AUDIO_SAMPLES_PER_TOKEN)
                ...
                if new_tokens[-1] < tokenizer.timestamp_begin:
                    ...
                    end_token = (
                        round(relative_end * SAMPLE_RATE / AUDIO_SAMPLES_PER_TOKEN)
                        + tokenizer.timestamp_begin
                    )
                ...
                next_seek = (
                    whisper_segments[i_segment + 1]["seek"]
                    if i_segment < len(whisper_segments) - 1
                    else None
                )
                if seek != next_seek:
                    ...
                    ), f"Got start {start} which is outside of audio duration {audio_duration}"
                    end = min(start + SEGMENT_DURATION, audio_duration)
                    tokens = current_tokens

            ##Callback function (deprecated)
            # if callback is not None:
            #     print("2nd callback after transcription")
            #     callback()
            ...
            with torch.no_grad():
                logprobs = model(
                    mfcc, torch.Tensor(tokens).int().to(model.device).unsqueeze(0)
                )
                logprobs = F.log_softmax(logprobs, dim=-1)

            end_token = tokenizer.timestamp_begin + round(
                min(N_FRAMES * HOP_LENGTH, end_sample - start_sample)
            ...
            ##Callback function (deprecated)
            # if callback is not None:
            #     print("3rd callback after transcription")
            #     callback()
            ...
            for word in ws:
                ...
                if trust_whisper_timestamps:
                    ...
                    assert (
                        not len(word["tokens_indices"])
                        or word["tokens_indices"][0] == tokens[i_token]
                    )
                    word.update({"idx_segment": token_to_idx_segment[i_token]})
                    i_token += len(word["tokens"])
                    while (
                        i_token < len(tokens)
                        and tokens[i_token] >= tokenizer.timestamp_begin
                    ):
                        i_token += 1
                ...
                if compute_word_confidence:
                    ...
                    word_logprobs = [
                        logprobs[:, step, tok]
                        for (step, tok) in zip(
                            range(i_start, i_start + len(tok_indices)), tok_indices
                        )
                    ]
                    i_start = i_end
            ...
            if trust_whisper_timestamps:
                if segment_tokens_check != segment["tokens"]:
                    assert (
                        len(segment_tokens_check) < len(segment["tokens"])
                        and segment_tokens_check[:-1]
                        == segment["tokens"][: len(segment_tokens_check) - 1]
    ...
    # print("Final Transcription Aligned! Writing to JSON...")
...
def get_audio_tensor(audio, device="cpu"):
    ...
    if isinstance(audio, np.ndarray):
        ...
        assert isinstance(
            audio, torch.Tensor
        ), f"Got unexpected audio of type {type(audio)}"
...
def get_tokenizer(model, task="transcribe", language="en"):
    try:
        return whisper.tokenizer.get_tokenizer(
            ...
            num_languages=(
                model.num_languages if hasattr(model, "num_languages") else 99
            ),
            task=task,
            language=language,
        )
        ...
        return whisper.tokenizer.get_tokenizer(
            model.is_multilingual, task=task, language=language
        )
...
def perform_word_alignment(
    ...
    if plot:
        ...
        plt.subplots(
            ...
            1,
            figsize=(16, 9),
            gridspec_kw={"height_ratios": [3] + [1] * (nplots - 1)},
        )
        ...
        xticklabels = [
            round_timestamp(x) for x in xticks * AUDIO_TIME_PER_TOKEN + start_time
        ]
        ...
        words_with_subwords = [
            "|".join(s).strip() for (w, s) in zip(words, word_tokens)
        ]
        ...
        if plot_mfcc:
            ...
            plt.imshow(
                mfcc[0, :, start_token * 2 : end_token * 2].cpu(),
                aspect="auto",
                origin="lower",
            )
            plt.yticks([])
        ...
        for i_token, (tok, begin, end) in enumerate(zip(tokens, jumps[:-1], jumps[1:])):
            ...
            if plot:
                ...
                for i, p in enumerate(peaks):
                    color = (
                        "red" if (len(peaks) > 1 and i < len(peaks) - 1) else "green"
                    )
                ...
                if "left_bases" in properties:
                    ...
                    barxxy(
                        begin + properties["left_bases"],
                        begin + properties["right_bases"],
                        properties.get(
                            "prominences", [1] * len(properties["left_bases"])
                        ),
                        alpha=0.5,
    ...
    if plot:
        ...
        for i, (w, ws, begin, end) in enumerate(
            zip(words, word_tokens, begin_times, end_times)
        ):
            ymax = ymin + len(ws)
            if mfcc is None:
                plt.text(
        ...
        if plot_mfcc:
            ...
            for i, (w, begin, end) in enumerate(zip(words, begin_times, end_times)):
                ...
                for x in [
                    ...
                    plt.axvline(
                        x * 2 / AUDIO_TIME_PER_TOKEN, color="red", linestyle="dotted"
                    )

        if isinstance(plot, str):
            plt.savefig(
                f"{plot}.alignment{num_alignment_for_plot:03d}.jpg",
                bbox_inches="tight",
                pad_inches=0,
            )
        else:
...
def split_tokens_on_unicode(
    ...
    for token in tokens:
        ...
        decoded = tokenizer.decode_with_timestamps(
            [
                t
                for t in current_tokens
                if t < tokenizer.eot or t >= tokenizer.timestamp_begin
            ]
        )
        if "\ufffd" not in decoded:
            ...
            previous_special = len(word_tokens_indices) > 0 and (
                word_tokens_indices[-1][-1] >= tokenizer.timestamp_begin
            )
...
def split_tokens_on_spaces(
    tokens: torch.Tensor, tokenizer, remove_punctuation_from_words=False
):
    subwords, subword_tokens_list, subword_tokens_indices_list = (
    ...
    for i, (subword, subword_tokens, subword_tokens_indices) in enumerate(
        ...
        previous_special = (i > 0) and (
            subword_tokens_indices_list[i - 1][0] >= tokenizer.timestamp_begin
        )
        next_special = (i < len(subword_tokens_indices_list) - 1) and (
            subword_tokens_indices_list[i + 1][0] >= tokenizer.timestamp_begin
        )
...
def remove_non_speech(
    ...
    if plot:
        ...
        times = [
            i * step / SAMPLE_RATE for i in range((audio.shape[-1] - 1) // step + 1)
        ]
...
def ensure_increasing_positions(segments, min_duration=0):
    ...
    for seg in segments:
        ...
        ), f"Got segment {seg} coming before the previous finishes ({previous_end} > {seg['start']})"
...
def write_csv(
    transcript, file, sep=",", text_first=True, format_timestamps=None, header=False
):
    writer = csv.writer(file, delimiter=sep)
    if format_timestamps is None:
        format_timestamps = lambda x: x
    if header is True:
...
def get_alignment_heads(model, max_top_layer=3):
    ...
    if num_parameters not in _PARAMETERS_TO_MODEL_NAME:
        logger.warning(
            "Could not retrieve alignment heads : taking all attention heads from the top layers"
        )
        return None
...
def load_model(
    ...
    if name in whisper.available_models() or extension == ".pt":
        return whisper.load_model(
            name, device=device, download_root=download_root, in_memory=in_memory
        )
    ...
    if extension in [".ckpt", ".bin"]:
        ...
        try:
            ...
            raise ImportError(
                f"If you are trying to download a HuggingFace model with {name}, please install first the transformers library"
            )
...
def states_to_dim(state_dict):
    ...
    return {
        ...
        "n_vocab": state_dict["decoder.token_embedding.weight"].shape[
            0
        ],  # 51864 / 51865
...
def cli():
    ...
    try:
        ...
        write_tsv = lambda transcript, file: write_csv(
            transcript,
            file,
            sep="\t",
            header=True,
    ...
    parser.add_argument(
        "--versions",
        help="show versions (of whisper-timestamped and whisper) and exit",
        action="version",
    ...
    parser.add_argument(
        ...
        "-f",
        default="all",
        help=f"Format(s) of the output file(s). Possible formats are: {', '.join(valid_formats)}. Several formats can be specified by using commas (ex: \"json,vtt,srt\"). By default (\"all\"), all available formats will be produced",
        type=str2output_formats,
    )
    ...
    parser.add_argument(
        "--language",
        help=f"language spoken in the audio, specify None to perform language detection.",
        choices=sorted(whisper.tokenizer.LANGUAGES.keys())
        + sorted([k.title() for k in whisper.tokenizer.TO_LANGUAGE_CODE.keys()]),
        default=None,
    )
    ...
    parser.add_argument(
        "--vad",
        default=False,
        help="whether to run Voice Activity Detection (VAD) to remove non-speech segment before applying Whisper model (removes hallucinations). Can be: True, False, silero, silero:3.1 (or another version), or autitok. Some additional libraries might be needed",
    )
    parser.add_argument(
        "--detect_disfluencies",
        default=False,
        help="whether to try to detect disfluencies, marking them as special words [*]",
        type=str2bool,
    )
    parser.add_argument(
        "--recompute_all_timestamps",
        default=not TRUST_WHISPER_TIMESTAMP_BY_DEFAULT,
        help="Do not rely at all on Whisper timestamps (Experimental option: did not bring any improvement, but could be useful in cases where Whipser segment timestamp are wrong by more than 0.5 seconds)",
        type=str2bool,
    )
    parser.add_argument(
        "--punctuations_with_words",
        default=True,
        help="whether to include punctuations in the words",
        type=str2bool,
    )

    parser.add_argument(
        "--temperature", default=0.0, help="temperature to use for sampling", type=float
    )
    parser.add_argument(
        "--best_of",
        type=optional_int,
        default=None if USE_EFFICIENT_BY_DEFAULT else 5,
    ...
    parser.add_argument(
        "--fp16",
        default=None,
        help="whether to perform inference in fp16; Automatic by default (True if GPU available, False otherwise)",
        type=str2bool,
    )

    parser.add_argument(
        "--temperature_increment_on_fallback",
        default=0.0 if USE_EFFICIENT_BY_DEFAULT else 0.2,
    ...
    parser.add_argument(
        "--compute_confidence",
        default=True,
        help="whether to compute confidence scores for words",
        type=str2bool,
    )
    parser.add_argument(
        "--verbose",
        type=str2bool,
        default=False,
        help="whether to print out the progress and debug messages of Whisper",
    )
    parser.add_argument(
        "--plot",
        help="plot word alignments (save the figures if an --output_dir is specified, otherwhise just show figures that have to be closed to continue)",
        default=False,
        action="store_true",
    )
    parser.add_argument(
        "--debug",
        help="print some debug information about word alignement",
        default=False,
        action="store_true",
    )
    ...
    parser.add_argument(
        "--naive",
        help="use naive approach, doing inference twice (once to get the transcription, once to get word timestamps and confidence scores).",
        default=False,
        action="store_true",
    )
    ...
    for audio_path in audio_files:
        ...
        outname = (
            os.path.join(output_dir, os.path.basename(audio_path))
            if output_dir
            else None
        )
        ...
        result = transcribe_timestamped(
            ...
            temperature=temperature,
            plot_word_alignment=(
                outname if (outname and plot_word_alignment) else plot_word_alignment
            ),
            **args,
        )
...
def filtered_keys(
    ...
    if isinstance(result, dict):
        return {
            k: (filtered_keys(v, keys) if k not in ["language_probs"] else v)
