...
if os.path.exists(required_packages_filename):
    install_requires2 = [
        l.strip() for l in open(required_packages_filename).readlines()
    ]
    assert (
        install_requires == install_requires2
    ), f"requirements.txt is not up-to-date: {install_requires} != {install_requires2}"
